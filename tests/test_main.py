...
def clean_db() -> None:
    ...
    try:
        with psycopg2.connect(DB_URL) as conn:
            with conn.cursor() as cursor:
                cursor.execute(clear_registered_users_query, )
                print("-----------Empty registered_users table-----------")
                cursor.execute(clear_login_users_query, )
                print("-----------Empty login_users table-----------")
                cursor.execute(clear_chat_messages_query, )
...
def test_registration_with_InvalidUserName() -> None:
    ...
    assert response.json() == {"detail": "This username is already in use, please choose another name."}
...
def test_verify_token() -> None:
    ...
    # Wrong token
    verify_token_response = auth_client.post("/verify_token", json = {'user_token': "aaa1222", 'user_name': USER1})
    assert verify_token_response.status_code == 200
...
def test_verify_token_UserNotFoundError() -> None:
    global user_1_token
    verify_token_response = auth_client.post("/verify_token", json = {'user_token': user_1_token, 'user_name': USER2})
    assert verify_token_response.status_code == 404
    assert verify_token_response.json() == {"detail": "Unidentified user, not registered or not logged in"}
...
def test_send_message_with_not_found_user() -> None:
    global user_1_token
    message_details = {'user_name': 'Noa', 'messages': ['Hi, how are you?'], 'destination_name': USER2}
    response = chat_client.post("/send_message", json = message_details, params = {'token': user_1_token})
    assert response.status_code == 404
...
def test_get_message_with_invalid_token():
    messages_response = chat_client.get("/get_messages", params = {'user_name':USER2, 'token': 'aaaa'})
...
def test_get_message_with_wrong_user_name():
    global user_2_token
    messages_response = chat_client.get("/get_messages", params = {'user_name':"Noa", 'token': user_2_token})
    assert messages_response.status_code == 404
    assert messages_response.json() == {"detail": "Unidentified user, not registered or not logged in"}
